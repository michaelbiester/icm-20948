...
# with some annotations to aid understanding

CHIP_ID = 0xEA
I2C_ADDR = 0x68
I2C_ADDR_ALT = 0x69
# for the selection of register banks: 0, 1, 2, 3
...
class ICM20948:
    ...
    def bank(self, value):
        ...
        if not self._bank == value:
            # sets bits 5/4 of register REG_BANK_SEL
    ...
    def mag_read(self, reg):
        ...
        self.bank(3)
        # set I2c address of magnetometer and set MSB = 1 for read operation
        self.write(ICM20948_I2C_SLV0_ADDR, AK09916_I2C_ADDR | 0x80)
        # set the (start) address from where to begin data transfer
        self.write(ICM20948_I2C_SLV0_REG, reg)
        # why ?
        self.write(ICM20948_I2C_SLV0_DO, 0xff)
        # enables read and set the number of bytes to read to 1 byte; data are stored in 
        # register EXT_SLV_SENS_DATA_00 . From there it must be read.
    ...
    def set_gyro_full_scale(self, scale=250):
        ...
        self.bank(2)
        # mask bits not related to scale
        value = self.read(ICM20948_GYRO_CONFIG_1) & 0b11111001
        # or operation inserts the two bits which determine the scale
    ...
    def __init__(self, i2c_addr=I2C_ADDR, i2c_bus=None):
        ...
        time.sleep(0.01)
        # from the data sheet: CLKSEL[2:0] should be set to 1~5 to achieve full gyroscope performance
        self.write(ICM20948_PWR_MGMT_1, 0x01)
        # accelerometer : all axis on; gyroscope: all axis on
        self.write(ICM20948_PWR_MGMT_2, 0x00)

        self.bank(2)

        # todo: understand what is exactly configured here ...
        ...
        # wait in this loop until the soft reset bit is set to "0" 
